...
<!DOCTYPE xsl:stylesheet[
   ...
   <!ENTITY start_date "substring-before(property:Has_start_time, 'T')">
]>
...
<xsl:key name="days" match="/rdf:RDF/swivt:Subject" use="&start_date;"/>
<!--xsl:key name="rooms" match="/rdf:RDF/swivt:Subject" use="&room_uri;"/-->
<xsl:key name="rooms-by-day" match="/rdf:RDF/swivt:Subject" use="concat(&start_date;, &room_uri;)"/>



<xsl:template match="/">
<schedule>
  <xsl:for-each select="/rdf:RDF/swivt:Subject[property:Has_start_time][generate-id() = generate-id(key('days', &start_date;)[1])]">
  <xsl:variable name="day"><xsl:value-of select="&start_date;"/></xsl:variable>
  <day>
    ...
    <room>
      ...
      <xsl:attribute name="name"><xsl:value-of select="$room"/></xsl:attribute><xsl:text>
      </xsl:text>

      <xsl:apply-templates select="key('rooms-by-day', concat(&start_date;, &room_uri;))" />
      <xsl:text>
    </xsl:text>
